...
from mapping import ship_keys, player_keys, cannon_keys
...
class SoTMemoryReader:
    ...
    def _read_name(self, actor_id: int) -> int:
        """
    ...
    def read_actors(self):
        ...
        for x in range(0, actor_data[1]):
            ...
            # as a ship
            if not self.actor_name_list.__contains__(raw_name):
                self.actor_name_list.append(raw_name)
                print(raw_name)
            ...
            elif CONFIG.get('PLAYERS_MARK_ENABLED') and raw_name in player_keys:
                player = Player(self.rm, actor_id, actor_address, self.my_coords,
                                raw_name)
                self.display_objects.append(player)
            elif CONFIG.get('CANNONS_ENABLED') and raw_name in cannon_keys:
                cannon = Cannon(self.rm, actor_id, actor_address, self.my_coords,
                                raw_name)
                self.display_objects.append(cannon)
